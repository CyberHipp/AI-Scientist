...
- Offline mock web fixture pack (HTML, JSON, RSS, paper-like pages) plus manifest, docs, and server utilities for deterministic tests without network calls, including edge cases (redirects, rate limits, compression, invalid JSON, timeouts, pagination, ETag/charset).
- Nature-style Perspective documenting anthrobot implications, EvalOps specs, and combination-product regulatory intersections for living-construct evaluation.
